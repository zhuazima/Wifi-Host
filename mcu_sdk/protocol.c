...
/* 
    ...
    mcu_dp_enum_update(DPID_MASTER_MODE,当前主机模式,子设备ID,子设备ID长度); //枚举型数据上报;
    mcu_dp_string_update(DPID_ALARM_ACTIVE,当前主动报警指针,当前主动报警数据长度,子设备ID,子设备ID长度); //STRING型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_alarm_active_handle
功能描述 : 针对DPID_ALARM_ACTIVE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
        : sub_id_buf:子设备id
        : sub_id_len:子设备id数据长度
...
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_alarm_active_handle(const unsigned char value[], unsigned short length, unsigned char *sub_id_buf, unsigned char sub_id_len)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*****************************************************************************
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_string_update(DPID_ALARM_ACTIVE,value, length, sub_id_buf, sub_id_len);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
    ...
    ****************************************************************/
    ...
    switch(dpid) {
        case DPID_MASTER_MODE:
            //主机模式处理函数
            ret = dp_download_master_mode_handle(value,length,sub_id_buf,sub_id_len);
        break;
        case DPID_ALARM_ACTIVE:
            //主动报警处理函数
            ret = dp_download_alarm_active_handle(value,length,sub_id_buf,sub_id_len);
        break;
